...
class LinearSolverFloat:
    ...
    # eq 3.25
        ...
        r_l_pot = 2**(- (l % self.R))
        return (4*r_l_pot * 
                sum(self.M[k,i_l] * (r_l_pot*self.M[k,i_l] - self.Y[k] - sum(self.M[k])) 
    ...
    # eq 3.26
        ...
        return (4*2**(-(r_l + r_m)) * 
                sum(self.M[k,i_l]*self.M[k,i_m] 
                    for k in range(0,self.N)))
    
    def _delta(self,i,l):
        return int(i == np.floor(l / self.R))
    def _get_x_i(self,i,q):
            return (2*sum(2**(-(l % self.R)) * q[l] * self._delta(i,l) 
    ...
    def sample(self):
        ...
        self.solutions = ([(np.array([self._get_x_i(i,q) for i in range(0,self.N)]),energy,num_occurrences) 
                           for q,energy,num_occurrences in self.sampleset.data(fields=['sample', 'energy', 'num_occurrences']) 
                           if num_occurrences >= self.min_occurrence])
        return self.sampleset
    def getSolutions(self, min_occurrence):
        if self.sampleset is None : return None
        return  ([(np.array([self._get_x_i(i,q) for i in range(0,self.N)]),energy,num_occurrences) 
                           for q,energy,num_occurrences in self.sampleset.data(fields=['sample', 'energy', 'num_occurrences']) 
