...
class RandomLP(RandomQuadraticProgram):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            num_constr, num_vars, name, multiple, penalty=penalty, boundaries=base_boundaries
        )
...
def create_models(
    path: "Union[list[str],str]", penalty: Optional[float] = None
) -> Dict[str, RandomQuadraticProgram]:
    """
    ...
    for path_ in paths:
        ...
        for file in filenames:
            ...
            qps[name] = RandomLP.create_from_lp_file(path_ + file, penalty=penalty)

    qps_sorted = OrderedDict()

    for qp_name in sorted(qps.keys(), key=lambda name: qps[name].complexity()):
